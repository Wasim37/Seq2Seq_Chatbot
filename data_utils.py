...
# dim 字典总长度6865，6861(dictionary.json内容长度) + 4(4个特殊标记EOS, UNK, PAD, GO)
# dictionary 加载的字典内容 ['<eos>', '<unk>', '<pad>', '<go>', '0', '1', '2', '3', '4......
# index_word OrderedDict([(0, '<eos>')])
# word_index OrderedDict([('<eos>', 0)])
...
def generate_bucket_dbs(
    ...
    # 有时候为什么生成元祖不生成列表？元祖不可以修改，所以有时候生成速度是列表的10倍
...
if __name__ == '__main__':
    # 将./db目录下的conversation.db数据整理成./bucket_dbs下的四个桶数据
